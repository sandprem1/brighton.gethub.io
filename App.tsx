...
import { Home, MapPin, BookOpen, Phone, FileText, Menu, X, Bell, CalendarCheck, Globe, PenTool, GraduationCap, Download, Settings, Check, Calendar, AlertTriangle, RefreshCw } from 'lucide-react';
...
const NavItem = React.memo(({ view, icon: Icon, label, active, onClick }: { view: ViewState; icon: any; label: string; active: boolean; onClick: () => void }) => (
    <button
      ...
      className={`flex items-center space-x-2 px-4 py-2 rounded-xl transition-all w-full md:w-auto text-left md:text-center nav-item ${
        active 
          ? 'bg-white/20 text-white font-bold backdrop-blur-md shadow-sm border border-white/20' 
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans relative overflow-x-hidden bg-gray-50 dark:bg-gray-900 transition-colors duration-500 selection:bg-primary-500/20 selection:text-primary-700">
      ...
      <div className="fixed inset-0 z-[-1] overflow-hidden transform-gpu pointer-events-none">
        <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-primary-50/40 via-white/90 to-blue-50/40 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950"></div>
        <div className="absolute top-[-20%] right-[-20%] w-[90vw] h-[90vw] bg-primary-400/20 dark:bg-primary-600/10 rounded-full blur-[120px] opacity-50 mix-blend-multiply dark:mix-blend-screen will-change-transform translate-z-0"></div>
        <div className="absolute bottom-[-20%] left-[-20%] w-[80vw] h-[80vw] bg-blue-400/15 dark:bg-blue-600/10 rounded-full blur-[120px] opacity-50 mix-blend-multiply dark:mix-blend-screen will-change-transform translate-z-0"></div>
      </div>

      <header className="bg-primary-700/85 dark:bg-black/70 backdrop-blur-xl backdrop-saturate-150 text-white sticky top-0 z-40 border-b border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.05)] transition-all duration-500 transform-gpu supports-[backdrop-filter]:bg-primary-700/60">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center cursor-pointer gap-3 group" onClick={() => setCurrentView(ViewState.HOME)}>
              <div className="relative w-10 h-10 flex items-center justify-center bg-white/15 rounded-xl backdrop-blur-sm shadow-inner ring-1 ring-white/20 transition-transform group-hover:scale-105 group-hover:bg-white/20">
                 <Globe className="text-white drop-shadow-sm" size={24} />
                 <div className="absolute -bottom-1 -right-1 bg-white rounded-full p-0.5 shadow-sm">
                    <PenTool className="text-primary-600 transform -rotate-12" size={10} fill="currentColor" /> 
                 </div>
              </div>
              <div className="flex flex-col justify-center">
                <h1 className="text-xl font-extrabold text-white leading-none tracking-tight drop-shadow-sm">Brighton</h1>
                <p className="text-[10px] text-primary-100 font-bold tracking-widest uppercase opacity-90 mt-0.5">Career Consultants</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3">
              {showInstallBtn && (
               ...
               <button onClick={() => setIsSettingsOpen(!isSettingsOpen)} className="p-2 text-white/80 hover:bg-white/10 rounded-full transition-all active:scale-95 hover:rotate-45 duration-300"><Settings size={20} /></button>
        ...
        {isSettingsOpen && (
          <div className="absolute top-16 right-4 w-64 bg-white/80 dark:bg-gray-800/80 backdrop-blur-2xl backdrop-saturate-150 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.15)] border border-white/50 dark:border-gray-600 p-4 animate-slideDown text-gray-800 dark:text-white z-50">
            ...
            <div>
              ...
              <div className="grid grid-cols-5 gap-2">
                {(['green', 'blue', 'purple', 'orange', 'rose'] as ThemeColor[]).map((color) => (
                  <button key={color} onClick={() => setThemeColor(color)} className={`w-8 h-8 rounded-full flex items-center justify-center transition-transform hover:scale-110 ${themeColor === color ? 'ring-2 ring-offset-2 ring-primary-500 dark:ring-offset-gray-800 scale-110' : ''}`} style={{ backgroundColor: `rgb(${COLOR_PALETTES[color][500]})` }}>{themeColor === color && <Check size={14} className="text-white" />}</button>
                ))}
              </div>
            </div>
          </div>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-primary-900/95 dark:bg-black/90 backdrop-blur-xl backdrop-saturate-150 border-t border-white/10 p-4 shadow-2xl absolute w-full z-50 max-h-[80vh] overflow-y-auto rounded-b-3xl animate-slideDown">
      ...
      {/* Enhanced Floating Bottom Dock - Apple Style */}
      <div className="md:hidden fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-white/70 dark:bg-black/60 backdrop-blur-2xl backdrop-saturate-150 border border-white/40 dark:border-white/10 rounded-[2.5rem] px-8 py-3 flex items-center justify-between space-x-6 shadow-[0_20px_50px_-10px_rgba(0,0,0,0.2)] ring-1 ring-white/50 dark:ring-white/5 z-30 max-w-[95%] w-auto animate-slideUp supports-[backdrop-filter]:bg-white/50">
        <button onClick={() => setCurrentView(ViewState.HOME)} className={`relative p-3 rounded-2xl transition-all duration-300 group flex flex-col items-center`}>
            <Home size={24} className={`transition-all duration-300 ${currentView === ViewState.HOME ? 'text-primary-600 dark:text-primary-400 -translate-y-1 scale-110' : 'text-gray-500 dark:text-gray-400'}`} strokeWidth={currentView === ViewState.HOME ? 2.5 : 2} />
            {currentView === ViewState.HOME && <span className="absolute -bottom-1 w-1 h-1 bg-primary-600 dark:bg-primary-400 rounded-full animate-fadeIn"></span>}
        </button>
        
        <button onClick={() => setCurrentView(ViewState.RESOURCES)} className={`relative p-3 rounded-2xl transition-all duration-300 group flex flex-col items-center`}>
            <CalendarCheck size={24} className={`transition-all duration-300 ${currentView === ViewState.RESOURCES ? 'text-primary-600 dark:text-primary-400 -translate-y-1 scale-110' : 'text-gray-500 dark:text-gray-400'}`} strokeWidth={currentView === ViewState.RESOURCES ? 2.5 : 2} />
            {currentView === ViewState.RESOURCES && <span className="absolute -bottom-1 w-1 h-1 bg-primary-600 dark:bg-primary-400 rounded-full animate-fadeIn"></span>}
        </button>

        {/* Center Action Button */}
        <button onClick={() => setCurrentView(ViewState.BOOKING)} className={`relative -top-6 p-4 rounded-[1.5rem] bg-gradient-to-tr from-primary-600 to-primary-400 text-white shadow-xl shadow-primary-600/40 border-[4px] border-gray-50/80 dark:border-gray-900/80 transition-transform active:scale-95 hover:scale-105 hover:shadow-primary-600/60 hover:-translate-y-7`}>
            <FileText size={26} strokeWidth={2.5} />
        </button>

        <button onClick={() => setCurrentView(ViewState.UK_INTERVIEW)} className={`relative p-3 rounded-2xl transition-all duration-300 group flex flex-col items-center`}>
            <GraduationCap size={24} className={`transition-all duration-300 ${currentView === ViewState.UK_INTERVIEW ? 'text-primary-600 dark:text-primary-400 -translate-y-1 scale-110' : 'text-gray-500 dark:text-gray-400'}`} strokeWidth={currentView === ViewState.UK_INTERVIEW ? 2.5 : 2} />
             {currentView === ViewState.UK_INTERVIEW && <span className="absolute -bottom-1 w-1 h-1 bg-primary-600 dark:bg-primary-400 rounded-full animate-fadeIn"></span>}
        </button>
        
        <button onClick={() => setCurrentView(ViewState.NEPALI_CALENDAR)} className={`relative p-3 rounded-2xl transition-all duration-300 group flex flex-col items-center`}>
            <Calendar size={24} className={`transition-all duration-300 ${currentView === ViewState.NEPALI_CALENDAR ? 'text-primary-600 dark:text-primary-400 -translate-y-1 scale-110' : 'text-gray-500 dark:text-gray-400'}`} strokeWidth={currentView === ViewState.NEPALI_CALENDAR ? 2.5 : 2} />
             {currentView === ViewState.NEPALI_CALENDAR && <span className="absolute -bottom-1 w-1 h-1 bg-primary-600 dark:bg-primary-400 rounded-full animate-fadeIn"></span>}
        </button>
      </div>
